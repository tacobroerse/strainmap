function hn=arrow3t(p1,p2,s,w,h,ip,alpha,beta)
% ARROW3 (R13)
...
if c1==2 && c2 == 2
    % add ones to third dimension to prevent clipping
    p1(:,3)=ones(r1,1);
    p2(:,3)=ones(r2,1);
    c2=3;
    c1=3;
...
%    end
  if lw==0                               % 02/09/2020
    ...
    for i=1:n
        % reduce length of vector otherwise it overlays with arrow
        diffx=p2(i,1)-p1(i,1);
        diffy=p2(i,2)-p1(i,2);
        diffz=p2(i,3)-p1(i,3);
    lengthvec=sqrt(diffx^2+diffy^2+diffz^2);
    
    lengtharrowhead = h(i)*sf;
    
    % define new end for line
    p3(i,1)=p1(i,1)+diffx*abs(lengthvec-lengtharrowhead/2)/lengthvec;
    p3(i,2)=p1(i,2)+diffy*abs(lengthvec-lengtharrowhead/2)/lengthvec;
    p3(i,3)=p1(i,3)+diffz*abs(lengthvec-lengtharrowhead/2)/lengthvec;
    
     H1(i)=plot3([p1(i,1),p3(i,1)],[p1(i,2),p3(i,2)],...
        [p1(i,3),p3(i,3)],ls{i},'Color',c(i,:),'LineWidth',lw(i));
